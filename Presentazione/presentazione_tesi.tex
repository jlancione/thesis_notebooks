...
\begin{frame}{Large Hadron Collider - CMS}
  \begin{figure}
    ...
    \begin{tikzpicture}
      ...
      \node [inner sep=0pt, text width=4cm] (text) at ($(cern) + (1.5,\the\paperheight/2)$) {Dummy text};
    \end{tikzpicture}
...
% \end{equation*}
  \begin{figure}
    \centering
    \begin{tikzpicture}
      \node [inner sep=0pt, text width=8cm, minimum height=5cm] (landscape) at (0,0) {
        \includegraphics[width=\textwidth]{./loss_landscape.pdf}
      };
      \node [inner sep=0pt, text width=5cm] (loss) at ($(landscape) + (-3,2)$) {
        \begin{equation*}
          H(p,q) = - \sum p(x) \log q(x)
        \end{equation*}
      };
